...
## Latest Verification
- Syntax: `node --check frontend/src/app/bootstrap.js`, `node --check frontend/src/pages/codex/index.js`, `node --check frontend/src/pages/codex/renderer.js`, `node --check scripts/capture-chatgpt-extension-source.cjs`, `node --check scripts/audit-codex-source-alignment.cjs`, `node --check scripts/audit-codex-dynamic-states.cjs`, `node --check scripts/audit-codex-dom-structure.cjs`, `node --check scripts/audit-codex-markup-alignment.cjs`, `node --check scripts/audit-codex-computed-styles.cjs`, `node --check scripts/verify-codex-panel-cdp.cjs`, `python -m py_compile scripts/verify-codex-panel-playwright.py`, and PowerShell parse check for `scripts/audit-codex-visual-diff.ps1`.
- Unified audit runner: `powershell -NoProfile -ExecutionPolicy Bypass -File scripts/run-codex-panel-audits.ps1 -CdpPort 9338` passed after the latest rebuild/restart; it runs source, workspace layout, DOM, markup, computed-style, and dynamic audits serially in a temporary Chrome.
