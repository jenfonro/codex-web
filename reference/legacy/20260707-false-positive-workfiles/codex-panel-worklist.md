...
## Boundary Rules
- Source of truth is captured Windows Chrome Codex extension DOM, class names, computed styles, and the live extension JS/CSS assets saved under `reference/extension-source/openai.chatgpt-26.5623.31443`.
- Do not invent a similar-looking UI when a captured structure or style exists.
- Reuse `frontend/src/assets/codex-panel/*.css` as exact copied extension CSS assets; local CSS is only for host isolation, static layout glue, and data-binding hooks.
- Keep the Codex panel in the left workspace sidebar.
- Reference captures and computed-style sampling must use a 1920x1080 or larger viewport unless a task explicitly records a different reference size.
- Before capturing code-server/Codex extension evidence, move the Codex extension to the left Activity Bar icon, open it from that icon, and close the right chat/auxiliary sidebar.
...
- For workspace-level drift, compare the full captured chrome class/DOM/computed-style category first. Fix proven shared causes such as font, layout flow, titlebar shadow, sidebar content background, and activity icon source; do not tune one visible symptom repeatedly.
...
- [x] Confirm extracted official extension CSS assets are present in `frontend/src/assets/codex-panel`.
...
- [x] Add shadow-only adapter CSS for frame sizing, static composer/thread layout, popovers, and codicon font isolation.
...
- [x] Rebuild, restart the controller from the latest Windows binary, and verify the served frontend assets match `frontend/src`.
...
- [x] Add auxiliary pixel visual-diff audit for panel and workspace screenshots without weakening source/DOM/markup/computed-style gates.
...
- [x] Restore the captured active sidebar sash color and placement from reference pixels.
...
- [x] Run and stabilize workspace layout audit against the captured Windows workspace reference.
...
- [x] Move the outer workspace layout from absolute-positioned stand-ins toward the captured static split-view geometry without changing the left-sidebar Codex panel behavior.
- [x] Re-run workspace/style/panel audits after rebuilding and record remaining mismatches by category instead of re-checking the same screenshot symptom.
...
- [x] Previous source audit used `frontend/src/assets/codex-panel` as the official CSS source, which allowed local corrupted assets to self-validate. Audit now compares copied CSS against `reference/extension-source/openai.chatgpt-26.5623.31443/webview/assets`.
...
- [x] Fixed overlay/toast sizing used the full browser viewport instead of the captured webview panel. Added a host containing block on `#codexPanel` and a narrow toast viewport adapter.
...
- [x] The captured active sidebar sash was gray/transparent locally while reference pixels at the sash are `rgb(0,105,204)`. Restored the 4px active blue sash on `.sidebar-resize-handle`.
...
- [x] Moved the outer workspace chrome out of `frontend/src/index.html` into `frontend/src/components/workspace/layout.js`; `index.html` now only owns the mount point and asset/script list.
- [x] Split the static Codex panel runtime into focused page files under `frontend/src/pages/codex/`: `config.js`, `utils.js`, `api.js`, `fixtures.js`, `index.js`, and `renderer.js`.
- [x] Replaced the stale explorer/demo bootstrap with `frontend/src/app/bootstrap.js`, which only owns fixed workspace platform class and sidebar resizing.
...
- [x] Removed the remote `sourceMappingURL` hint from the local workbench CSS so local devtools do not reference `main.vscode-cdn.net`.
...
- [x] Static serving now distinguishes client routes from missing assets: extensionless routes such as `/nodes` and `/runs/active` fall back to `index.html`, while missing JS/CSS/image assets return `404` with `Cache-Control: no-store`.
- [x] Split Codex fixture/sample data and fixture-only preview session creation out of the production `app/codex-web.js` bundle into `app/codex-fixtures.js`. Normal runtime no longer requires `CodexPanelFixtures`; explicit `?codexFixture=...` audit/dev URLs load it on demand with the same asset-version cache buster.
- [x] Protected `app/codex-fixtures.js` behind `CODEX_WEB_ENABLE_FIXTURES=1`. Production/default static serving returns `404` with `Cache-Control: no-store`; local audit mode can still enable the bundle explicitly. The server normalizes request paths before this check, so `./`, repeated slash, and URL-encoded path variants are blocked too.
- [x] Made `test-go.sh` fresh-clone safe for the backend `go:embed all:dist` boundary: when `backend/public/dist/index.html` is missing, it builds `frontend/dist` and syncs it before running backend and agent Go tests.
- [x] Static directories are explicitly blocked: `/app/`, `/assets/`, and `/assets/codex-panel/` return `404` with `Cache-Control: no-store`, while `/` still serves `index.html`.
- [x] Added virtual-scroll audit coverage to the unified panel audit runner so long-session blank windows, overlap, missing summaries, grouped commands, and file references cannot regress silently.
- [x] Added Runs view browser audit coverage to the unified panel audit runner so controller-owned run aggregation, all-session SSE subscription, cancel routing, and open-run navigation are executed, not just source-checked.
- [x] Added Nodes/Workspace/Git browser audit coverage to the unified panel audit runner so active-node selection, offline-node deletion, workspace requests, and git requests are executed with selected-node evidence.
- [x] Rebuilt, restarted, and re-ran source, workspace layout, DOM, markup, computed, dynamic, Go, screenshot, and visual-diff verification after the refactor.

## Latest Verification
- Syntax and audit orchestration: `powershell -NoProfile -ExecutionPolicy Bypass -File scripts/run-codex-panel-audits.ps1` passed from a clean temporary Chrome. The runner performs Node syntax checks for the Codex page/runtime modules and every audit script, Python syntax checks for workspace/Playwright helpers, and then runs the source, session sequencing, SSE reconnect, system architecture, workspace layout, Runs view, controller side views, DOM, markup, computed-style, dynamic-state, event-mapping, virtual-scroll, tool-shell, disclosure-collapse, disclosure-anchor, file-diff, collapse-capture, and collapse-window-rule audits.
- Unified audit runner default: `http://127.0.0.1:58888/?codexFixture=reference`. Normal production runtime still uses agent-backed API data; the fixture is explicit audit input only. The runner now preflights `/app/codex-fixtures.js` and requires the local service to be started with `CODEX_WEB_ENABLE_FIXTURES=1`.
...
- Source audit: `reference/codex-reference/source-alignment-audit.md`, generated `2026-07-05T23:32:17.845Z`, unexplained class/data/CSS counts all `0`, copied extension CSS non-exact count `0`.
- DOM structure audit: `reference/codex-reference/dom-structure-audit.md`, generated `2026-07-05T23:32:21.885Z`; every tracked component is `exact` or source-backed `compatible`, and anything outside those statuses fails the command.
- Markup audit: `reference/codex-reference/markup-alignment-audit.md`, generated `2026-07-05T23:32:24.728Z`; every tracked component is `exact` or source-backed `compatible`, and anything outside those statuses fails the command.
- Computed-style audit: `reference/codex-reference/computed-style-audit.md`, generated `2026-07-05T23:32:31.860Z`; actionable differences `0`, environment-size differences `0`, total differences `0`, missing elements `0`.
- Build: `./build-all.sh` from Git Bash on Windows; generated `build/codex-web.exe` and `build/codex-agent.exe`.
- Runtime: local `build/codex-web.exe` restarted after build, serving `http://127.0.0.1:58888/` from PID `45036`; after audit it was returned to default fixture-disabled mode.
- Static fallback access: default local mode returned `200`/`no-cache`/`text/html` for `/`, `/nodes`, and `/runs/active`; returned `404`/`no-store` for `/app/`, `/assets/`, `/assets/codex-panel/`, `/app/missing.js`, and `/assets/missing.png`; served cleaned existing asset `/app/./codex-web.js` and `/app/codex-web.js` as JavaScript with public cache.
- Static fixture access: default local mode returned `404` for `/app/codex-fixtures.js`, `/app/./codex-fixtures.js`, `/app//codex-fixtures.js`, and `/app/%63odex-fixtures.js`; audit mode with `CODEX_WEB_ENABLE_FIXTURES=1` returned `200` and loaded `defineCodexPanelFixtures`.
- Tests: `go test ./...` in `backend` and `agent` passed. Fresh-clone simulation also passed: temporarily moving `backend/public/dist` away caused `test-go.sh` to build the frontend dist, recreate `backend/public/dist`, and complete backend plus agent Go tests.
...
- Dynamic state audit: `reference/codex-reference/dynamic-state-audit.md`, generated `2026-07-05T23:32:34.050Z`; checks `thinking-shimmer`, cadenced shimmer sweep/highlight/active class, running `group/activity-header`, expanded disclosure body, completed/failed/cancelled state transitions, composer placeholder/send states, and source snippets from official extension assets; failed checks `0`.
- Current expanded audit coverage: session sequencing `7/7`, SSE reconnect `9/9`, system architecture `43/43`, Runs view `12` checks/`0` failed, controller side views `16` checks/`0` failed, event mapping `32` checks/`0` failed, virtual scroll `12` checks/`0` failed, tool shell `23` checks/`0` failed, disclosure collapse `27` checks/`0` failed, disclosure anchor `7` checks/`0` failed, file/diff `39` checks/`0` failed, collapse capture `31` checks/`0` failed, and collapse window rules `25` checks/`0` failed.
- Production bundle/static audit: `reference/codex-reference/system-architecture-audit.md`, generated `2026-07-06T02:34:18.410Z`, verifies `backend/public/dist/app/codex-web.js` excludes fixture definitions/sample data and fixture-only preview session creation, `backend/public/dist/app/codex-fixtures.js` contains the explicit fixture runtime, the controller hides that fixture asset unless `CODEX_WEB_ENABLE_FIXTURES=1` is set, static asset misses and static directories return 404 while client routes fall back to `index.html`, and `test-go.sh` prepares embedded frontend dist for fresh clones.
- Workspace layout audit: `reference/codex-reference/workspace-layout-audit.md`, generated after the workspace-root fix; tracked rows `23`, actionable differences `0`, missing rows `0`. This removed the previous shared drift categories: `Microsoft YaHei` fallback inheritance, absolute part positioning, titlebar shadow, sidebar content background/overflow, activity item color/font, and the hand-made Codex activity icon.
...
- Shadow DOM boundary to prevent extension Tailwind/base CSS from mutating the workspace chrome.
