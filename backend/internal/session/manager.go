...
func (m *Manager) refreshFromDisk() {
	...
	titleIndex := loadHistoryTitleIndex(m.cfg.CodexHome)
	m.mu.Lock()
	...
	for _, file := range files {
		if id := m.sessionIDForPathLocked(file.path); id != "" {
			...
			if existing != nil {
				applyManagedSessionTitleIndex(existing, titleIndex)
			}
	...
	for _, file := range pending {
		history, err := parseHistoryIndexWithTitles(file.path, titleIndex)
...
func applyManagedSessionTitleIndex(session *managedSession, titleIndex map[string]historyTitleEntry) {
	if session == nil {
		return
	}
	if title, ok := historyTitleForRecord(session.record, titleIndex); ok {
