...
type historyTitleEntry struct {
	Title     string
	UpdatedAt time.Time
}

type historyTitleIndexLine struct {
	ID         string `json:"id"`
...
func parseHistoryIndex(path string) (parsedSession, error) {
	return parseHistoryIndexWithTitles(path, nil)
}

func parseHistoryIndexWithTitles(path string, titleIndex map[string]historyTitleEntry) (parsedSession, error) {
	...
	if !applyHistoryTitleIndex(&record, titleIndex) && firstUserText != "" {
...
func loadHistoryTitleIndex(codexHome string) map[string]historyTitleEntry {
	paths, err := filepath.Glob(filepath.Join(codexHome, "session_index*.jsonl"))
	if err != nil || len(paths) == 0 {
		return nil
	}
	sort.Strings(paths)
	titles := map[string]historyTitleEntry{}
	for _, path := range paths {
		readHistoryTitleIndexFile(path, titles)
	}
	return titles
}

func readHistoryTitleIndexFile(path string, titles map[string]historyTitleEntry) {
	file, err := os.Open(path)
	if err != nil {
		return
	}
	...
	for scanner.Scan() {
		...
		var entry historyTitleIndexLine
		if err := json.Unmarshal(line, &entry); err != nil {
			continue
		}
		...
		updatedAt := parseHistoryTitleUpdatedAt(entry.UpdatedAt)
		existing, ok := titles[id]
		if !ok || existing.UpdatedAt.Before(updatedAt) || existing.Title == "" {
			titles[id] = historyTitleEntry{Title: title, UpdatedAt: updatedAt}
		}
	}
}

func parseHistoryTitleUpdatedAt(value string) time.Time {
...
func applyHistoryTitleIndex(record *model.SessionRecord, titles map[string]historyTitleEntry) bool {
	title, ok := historyTitleForRecord(*record, titles)
	if !ok {
		return false
	}
	record.Title = title
...
func historyTitleForRecord(record model.SessionRecord, titles map[string]historyTitleEntry) (string, bool) {
	if len(titles) == 0 {
		return "", false
	}
	for _, id := range []string{record.CodexThreadID, record.ID} {
		id = strings.TrimSpace(id)
		if id == "" {
			continue
		}
		if entry, ok := titles[id]; ok && strings.TrimSpace(entry.Title) != "" {
			return strings.TrimSpace(entry.Title), true
		}
	}
	return "", false
}
